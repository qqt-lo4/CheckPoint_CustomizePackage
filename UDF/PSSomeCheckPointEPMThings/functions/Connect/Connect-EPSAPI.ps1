function Connect-EPSAPI {
    ...
    Process {
        $iwr = Invoke-GraphQLQuery -Uri $sURL -OperationName $sOperationName -Query $sQuery -Variables $hVariables -IgnoreSSLError:$ignoreSSLError.IsPresent -Raw
        if ($null -ne $iwr) {
            ...
            $oResult | Add-Member -MemberType ScriptMethod -Name "Reconnect" -Value {
                ...
                $iwr = Invoke-GraphQLQuery -Uri $this.BaseURL -OperationName $sOperationName -Query $sQuery -Variables $hVariables -IgnoreSSLError:$this.IgnoreSSLError -Raw
            ...
            $oResult | Add-Member -MemberType ScriptMethod -Name "CallAPI" -Value {
                ...
                $oAPICall = Invoke-GraphQLQuery -Uri $this.BaseURL -IgnoreSSLError:$this.IgnoreSSLError `
                                          -Query $Query -OperationName $OperationName -Variables $Variables -Headers @{token = $this.Token} -Raw
                if ($oAPICall.errors) {
                    $this.Reconnect()
                    $oAPICall = Invoke-GraphQLQuery -Uri $this.BaseURL -IgnoreSSLError:$this.IgnoreSSLError `
                                        -Query $Query -OperationName $OperationName -Variables $Variables -Headers @{token = $this.Token} -Raw
