<#
.SYNOPSIS
    Class for managing Check Point trac_defaults configuration file

.DESCRIPTION
    Manages the trac_defaults configuration file for Check Point Endpoint Security.
    Provides methods to read, modify, backup, and save configuration settings.
    Maintains the original file formatting and whitespace.

.METHODS
    - HasOption([string]$optionname): Checks if an option exists
    - SetOptionValue([string]$optionname, [string]$value): Sets an option value
    - GetOptionValue([string]$optionname): Gets an option value
    - ToString(): Converts settings to string representation
    - Backup(): Creates a timestamped backup of the configuration file
    - Save(): Saves the current settings to the file (after creating a backup)
